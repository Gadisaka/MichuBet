# External Payment Verification Integration Guide (MichuBet)

End-to-end documentation for how MichuBet verifies **CBE**, **Telebirr**, and **CBE Birr** transfers via an external Verifier API, then credits a player wallet **once** per payment reference.
...
5. [File map (MichuBet)](#5-file-map-michubet)
...
22. [Full source code (as used in MichuBet)](#22-full-source-code-as-used-in-michubet)
...
### What MichuBet does with a successful verify
...
```text
┌─────────────────────┐     JWT + permission      ┌──────────────────────────────┐
│  Player frontend    │ ─────────────────────────► │  MichuBet backend            │
...
MichuBet success rule: `body.success === true`  
...
MichuBet success rule: `transactionStatus` (case-insensitive) === `"completed"`  
...
Upstream notes that Telebirr receipt pages often block foreign IPs. If Telebirr verifies fail from your VPS, host the verifier (or its proxy) inside Ethiopia, or use their documented fallback. Our MichuBet client simply surfaces network/API errors as deposit failures.
...
Use the cURL samples in §6.4 with a known good receipt before blaming MichuBet.
...
## 22. Full source code (as used in MichuBet)

The following is the complete application code that implements this integration (copied from the repo for porting). Paths are relative to the MichuBet monorepo root.
...
````markdown
# Prompt: Integrate Ethiopian external payment verification (CBE / Telebirr / CBE Birr) like MichuBet
...
Follow the full MichuBet reference doc attached in this conversation (`docs/external-payment-verification-integration.md`) for exact code, response field names, and edge cases. Match behavior unless the host stack requires an equivalent adaptation — do not invent alternate success/amount field names.
````
...
*Document generated from the MichuBet codebase integration of the Leul/Creofam Verifier API (CBE / Telebirr / CBE Birr).*
