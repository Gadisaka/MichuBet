# InOut Games Integration — MichuBet

Complete reference for the **seamless-wallet** integration with [InOut Games](https://inout.games). MichuBet holds player balances; InOut calls our webhook for `init`, `bet`, `withdraw`, and `rollback`. Games launch in an iframe via InOut’s launch URL.

**Production callback URL:** `https://api.michot.bet/api/integrations/inout/webhook`  
**Currency:** ETB (Ethiopian Birr)  
**Brand:** MichuBet / michotbet-prod  

> This document covers the **production** backend (`api.michot.bet`). We use one backend for both InOut certification and live play — no separate staging stack on the VPS.
...
| Topic | Choice |
|--------|--------|
| Wallet model | Seamless — MichuBet debits/credits ETB on webhook |
...
Maps InOut `authToken` → MichuBet user for webhooks.
...
curl -s -o /dev/null -w "%{http_code}\n" \
  -X POST https://api.michot.bet/api/integrations/inout/webhook
```
...
```bash
cd ~/michotbet/admin && npm run build
```
...
```bash
cd ~/michotbet/backend

BASE_URL="https://api.michot.bet" \
...
```
Webhook:     POST https://api.michot.bet/api/integrations/inout/webhook
Player lobby: https://michot.bet/casino
