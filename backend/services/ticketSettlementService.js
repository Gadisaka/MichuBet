...
async function gradeSelectionsInTx(tx, selections, matchResults, marketResultOverrides, context = {}) {
  ...
  for (const sel of selections) {
    ...
    // Unpaid OPEN prebooks are not settleable yet — leave legs PENDING.
    if (ownerTicket && isUnpaidOpenTicket(ownerTicket)) {
      continue;
    }
    // EXPIRED tickets are dead: leftover pending legs cannot change the
    // outcome, but they must resolve or the fixture stays stuck. Do not
    // add to ticketIds (no recompute / no cashback). Already-LOST tickets
    // still fall through so remaining legs grade and deferred cashback can
    // credit once every leg has a result.
    if (
      ownerTicket &&
      ownerTicket.status === "EXPIRED" &&
