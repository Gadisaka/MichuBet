...
function TileArt({ tile, imageUrl }) {
  if (tile.kind === "brand") {
    return (
      <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_50%_35%,rgba(246,175,1,0.22),transparent_70%)]">
        <span className="text-[15px] font-black lowercase tracking-tight text-[#F6AF01] sm:text-[17px]">
          inout
        </span>
      </div>
    );
        ...
        {tiles.map((tile) => {
          ...
          return (
            <button
              ...
              onClick={() => navigate(to)}
              title={label}
              className={`group relative flex w-[86px] shrink-0 snap-start cursor-pointer flex-col items-center gap-1.5 overflow-hidden rounded-2xl border bg-[#0a0a0a] p-2 pt-3 transition-all hover:ring-1 hover:ring-[#F6AF01]/60 sm:w-[100px] ${
                tile.pinned
                  ? "border-[#F6AF01]"
                  : "border-(--sb-accent-border) hover:border-[#F6AF01]/50"
              }`}
            >
              {tile.pinned ? (
                <span className="absolute left-0 top-0 flex items-center gap-0.5 rounded-br-lg bg-[#F6AF01] px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wide text-[#000000]">
