...
$PrinterName = $env:MICHOTBET_PRINTER_NAME
if ([string]::IsNullOrWhiteSpace($PrinterName)) {
    Write-Error "Missing MICHOTBET_PRINTER_NAME"
    exit 2
}
...
public class RawPrinterHelper {
    ...
    public static int SendBytesToPrinter(string szPrinterName, byte[] bytes) {
        ...
        if (!OpenPrinter(szPrinterName, out hPrinter, IntPtr.Zero))
            return -1;
        try {
            var di = new DOCINFOW { pDocName = "Michotbet Ticket", pDataType = "RAW" };
...
export function printRawViaPowerShell(buffer, printerName) {
  ...
  return new Promise((resolve) => {
    const child = spawn(
      ...
      {
        windowsHide: true,
        env: { ...process.env, MICHOTBET_PRINTER_NAME: printerName },
      },
    );
